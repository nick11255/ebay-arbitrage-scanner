...
def calculate_fees(sell_price: float, shipping_cost: float = None) -> dict:
...
def calculate_profit(buy_price: float, sell_price: float, shipping_cost: float = None) -> dict:
...
def score_deal(
    ...
    """
    avg_price = comp_stats.get("avg_price", 0)
    if avg_price <= 0:
        return {
    ...
    profit_info = calculate_profit(buy_price, avg_price)

    # Profit margin score (0-35)
    margin = profit_info["margin"]
    if margin >= 40:
        margin_score = 35
    elif margin >= 30:
        margin_score = 30
    ...
    sell_through = comp_stats.get("sell_through", "low")
    sell_scores = {"high": 25, "medium": 15, "low": 5}
    sell_score = sell_scores.get(sell_through, 5)

    # Comp confidence score (0-15)
    num_comps = comp_stats.get("num_comps", 0)
    if num_comps >= 20:
        comp_score = 15
    elif num_comps >= 10:
        comp_score = 12
    ...
    seller_pts = min(15, int(seller_score / 100 * 15))

    # Description quality score (0-10)
    desc_score = max(0, 10 - red_flags * 3)

    total = margin_score + sell_score + comp_score + seller_pts + desc_score
...
def format_deal_summary(item: dict, deal: dict) -> str:
